    ...
    UI.component('datepicker', {
        ...
        defaults: {
            ...
            i18n: {
                ...
                weekdays: ['جمعه', 'شنبه', 'یکشنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنجشنبه']
            },
            ...
            template: function (data, opts) {
                ...
                if (UI.formSelect) {
                    ...
                    var currentyear = moment().jYear(), options = [], months, years, minYear, maxYear;
                    ...
                    minYear = data.minDate ? data.minDate.year() : currentyear - 2;
        ...
        pick: function (initdate) {
            ...
            this.initdate = this.current.format("jYYYY-jMM-jDD");
        ...
        setMonth: function (month) {
            this.current.jMonth(month);
            this.update();
        },

        setYear: function (year) {
            this.current.jYear(year);
            this.update();
        },

        update: function () {

            var data = this.getRows(this.current.jYear(), this.current.jMonth()),
        ...
        jDay: function (day) {
            var georgian = moment.jConvert.toGregorian(1394, 1, 1);
            var gD = new Date(georgian.gy, georgian.gm, georgian.gd);
            var firstDay = gD.getDay() + 3;
        ...
        getRows: function (year, month) {
            ...
            var opts = this.options,
                ...
                days = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, (year % 4 === 0 && year % 100 !== 0 || year % 400 === 0) ? 30 : 29][month],
                before = new Date(year, month, 1, 12).getDay(),
                data = { "month": month, "year": year, "weekdays": [], "days": [], "maxDate": false, "minDate": false },
                row = [];
            var zz = new Date(parseInt(now.split('-')[0]), parseInt(now.split('-')[1]), parseInt(now.split('-')[2]));
            var xx = moment(zz);
            before = this.jDay(xx.dayOfYear());
            if (before > 6) {
                before -= 6;
            }
